...
import { fetchDoc } from "@/app/(frontend)/(api)/fetchDoc";
import Link from "next/link";
...
export default async function ProductPage(props: {
  ...
  const reviews: ProductReviewResponse = await fetchDoc<ProductReviewResponse>({
    collection: "reviews",
    id: product.id,
  });
          ...
          <div className="flex flex-col rounded-lg border border-neutral-200 bg-white p-8 md:p-12 lg:flex-row lg:gap-8 dark:border-neutral-800 dark:bg-black">
            ...
            <div className="basis-full lg:basis-2/6">
              <Suspense fallback={null}>
                <ProductDescription product={product} reviews={reviews.docs} />
              </Suspense>
            </div>
          </div>
...
async function RelatedProducts({
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/related-products`,
      {
        method: "POST",
